...
const playGame = () => {
    ...
    typedWords.value = ''; // Clear the previous text in the textbox
    let date = new Date();
...
const endGame = () => {
    ...
    let typedWordsArray = totalStr.split(" ");
    wordCount = typedWordsArray.length;
    for (let i = 0; i < wordCount; i++) {
        if (typedWordsArray[i] !== sentences[randomNumber].split(" ")[i]) {
            mistakeCount++;
        }
    }
    let correctCount = wordCount - mistakeCount;
    let finalMsg = `You Typed Total ${wordCount} words in ${totalTime} seconds. 
    Correctly Typed Words: ${correctCount}
    Mistake Words: ${mistakeCount}`;
